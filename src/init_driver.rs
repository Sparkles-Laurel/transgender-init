...
/// 
/// this is going to be stashed after switching to Kanit as a base.
#[derive(Clone)]
pub struct InitArgs {
    /// Name of the target to initialize into
    /// If not specified we are going to assume
    /// either "graphical" or "multi-user"
    /// depending on the configuration of Transgender.
    pub target_name: String,

    /// The rootfs device name
    /// must be a devtmpfs path
    pub rootfs: String,
    /// The Transgender command line
    pub cmdline: String,
    /// Location of the Androgen corpus
...
impl InitDriver {
    ...
    pub fn new(args: InitArgs) -> InitDriver {
        // load os-release
        ...
        InitDriver {
            ...
            corpus: vec![],
            corpus_path: args.init_corpus.clone(),
            args: args,
        }
    }
}
...
impl InitDriver {
    ...
    pub fn load_corpus(&mut self) -> Result<(), ()> {
...
impl Unit for InitDriver {
    fn conduct(&mut self, _args: InitArgs) -> Result<(), String> {
